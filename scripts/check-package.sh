...
# Everything the script needs at runtime.
if has "$VENDOR/$NAME.js"; then note "entry script is in place"; else bad "no $VENDOR/$NAME.js"; fi
for lib in $( cd "$( dirname "$0" )/.." && ls pjsr/lib ); do
   if has "$VENDOR/lib/$lib"; then note "lib/$lib is packaged"; else bad "lib/$lib is missing from the package"; fi
done
...
# upstream CC BY-NC 4.0 work requires its attribution to travel with every copy,
# and for most users the zip is the only thing they ever see.
...
# The version stamp has to have been substituted, or the dialog shows the
# literal placeholder and no user can report which build they are on.
if unzip -p "$ZIP" "$VENDOR/$NAME.js" | grep -q '__BUILD__'; then
   bad "the __BUILD__ placeholder was not stamped"
else
